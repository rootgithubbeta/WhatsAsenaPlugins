...
Asena.addCommand({pattern: 'utst ?(.*)', fromMe: true, desc: 'Send message for testing' }, (async (message, match) => { 
	if (match[1] === '') return await message.client.sendMessage(message.jid,'message;type(text,video,audio,document,url)',MessageType.text);
	try{
		arg = match[1].split(';');
		if (arg[1] === 'url') return await message.client.sendMessage(message.jid,{ url: arg[0] },MessageType.document,{ mimetype: Mimetype.application, caption: "Your file!" });
